...
function TopRecruiters({data,onUpdate,userRole}) {
  // Automatically scoped to the current Primerica month — no manual "Clear" needed
  // anymore, it just naturally moves to the new month once the cutoff passes.
  const pmForRecruiters = getCurrentPrimerMonth(data.primerMonthEnds||[]);
  const allPeople = [
    ...(data.admins||[]).map(p=>({...p,role:"Admin"})),
    ...(data.trainers||[]).map(p=>({...p,role:"Trainer"})),
    ...activeReps(data.reps).map(p=>({...p,role:"Rep"})),
  ...
  const recruitCounts = allPeople.map(p=>({
    ...p,
    // Only count recruits added this Primerica month — nothing about the reps
    // themselves (or who recruited them) is ever deleted, just what's shown here.
    recruits:(data.reps||[]).filter(r=>r.recruitedBy===p.id&&!r.excludeFromRecruitCount&&r.createdAt&&localDateStr(new Date(r.createdAt))>=pmForRecruiters.start),
  ...
  return <Card style={{marginBottom:14}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
      <div style={{fontSize:14,fontWeight:700,color:C.text}}>Top Recruiters<span style={{fontSize:11,fontWeight:500,color:C.textLight}}> ({pmForRecruiters.label})</span></div>
    </div>
