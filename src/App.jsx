...
function Sidebar({section,onNav,role,name,onSignOut,onClose,onShowPhone,onShowTour,alsoRecruits=false}) {
  const nav=[
    ...
    {k:"objectiontraining",l:"Objection Training",d:"M12 22C6.477 22 2 17.523 2 12S6.477 2 12 2s10 4.477 10 10-4.477 10-10 10zm-1-7v2h2v-2h-2zm2-1.645A3.502 3.502 0 0012 8.5a3.5 3.5 0 00-3.433 2.813l1.962.393A1.5 1.5 0 1113.5 13c0 .827-.673 1.5-1.5 1.5v.855z"},
...
// ── OBJECTION TRAINING PAGE ──
const OBJECTION_SCENARIOS = [
  {id:"cant_afford",cat:"Life Insurance",emoji:"💰",title:"I can't afford it",prospect:"I hear you, but honestly we're really tight on money right now. I just don't think we can afford another bill."},
  {id:"think_about",cat:"Life Insurance",emoji:"⏰",title:"I need to think about it",prospect:"This all sounds good but I just need some time to think about it. Can I get back to you?"},
  {id:"spouse",cat:"Life Insurance",emoji:"👫",title:"I need to talk to my spouse",prospect:"I can't make this decision without my spouse. They handle all our finances."},
  {id:"have_insurance",cat:"Life Insurance",emoji:"📋",title:"I already have insurance through work",prospect:"I actually already have life insurance through my job so I think I'm covered."},
  {id:"too_young",cat:"Life Insurance",emoji:"🧑",title:"I'm too young to worry about that",prospect:"I'm only 28, I don't really need life insurance right now. That's something for older people."},
  {id:"dont_believe",cat:"Life Insurance",emoji:"🤔",title:"I don't believe in life insurance",prospect:"I just don't believe in life insurance. If something happens to me, my family will figure it out."},
  {id:"market",cat:"Investments",emoji:"📈",title:"I don't trust the stock market",prospect:"After what happened in 2008, I just don't trust putting my money in the market. I'd rather keep it in savings."},
  {id:"401k",cat:"Investments",emoji:"💼",title:"I already have a 401k",prospect:"I already contribute to my 401k at work. I think that's enough for retirement."},
  {id:"no_money_invest",cat:"Investments",emoji:"💸",title:"I don't have extra money to invest",prospect:"I live paycheck to paycheck. There's just no extra money to put toward investments."},
  {id:"pyramid",cat:"Recruiting",emoji:"🔺",title:"Is this a pyramid scheme?",prospect:"No offense but this sounds like one of those pyramid schemes. My cousin got involved in something like this and lost money."},
  {id:"no_time",cat:"Recruiting",emoji:"⌚",title:"I don't have time for this",prospect:"I already work 50 hours a week. I just don't have the time to build a business on the side."},
  {id:"dont_know_people",cat:"Recruiting",emoji:"👥",title:"I don't know enough people",prospect:"I'm pretty introverted. I don't really have a big network so I don't see how I'd be successful."},
  {id:"costs_money",cat:"Recruiting",emoji:"💵",title:"How much does it cost to join?",prospect:"Wait, I have to pay $99 to join? Why would I pay to work for a company?"},
  {id:"tried_mlm",cat:"Recruiting",emoji:"❌",title:"I've tried MLM before and it didn't work",prospect:"I tried one of those network marketing things before and I ended up losing money and friends. I'm not interested."},
];

const PRIMERICA_CONTEXT = `You are roleplaying as a prospect in a Primerica training simulation. Your role is to play a realistic, skeptical but not hostile prospect that a Primerica financial representative is speaking with.

ABOUT PRIMERICA (know this to respond authentically):
- Primerica sells term life insurance only — "buy term and invest the difference" philosophy
- They serve middle-income families, not wealthy clients
- Reps are independent contractors (1099)
- The FNA (Financial Needs Analysis) is the main tool for presenting needs
- $99 IBA fee to join as a rep
- Reps need a state life insurance license to write business
- Company is publicly traded on NYSE (PRI), founded 1977
- Policy fee is $65
- Compensation ranges from Rep (25%) to RVP (110%)

YOUR BEHAVIOR AS THE PROSPECT:
- Stay in character as a real person with real concerns
- If the rep gives a good, empathetic, logical response — soften slightly but don't cave immediately
- If the rep gives a weak, pushy, or generic response — push back more
- Ask follow-up questions a real prospect would ask
...
- After 4-5 exchanges, naturally wrap up the conversation (either more open to discussing further, or politely declining for now)

When the conversation is complete (after 4-5 exchanges), end your response with exactly: [CONVERSATION_COMPLETE]`;

function ObjectionTrainingPage({data,onUpdate,session}) {
  const [mode,setMode]=useState("home"); // home | practice | library | review
  ...
  const [libraryInput,setLibraryInput]=useState({objection:"",response:""});
  const [generatingLibrary,setGeneratingLibrary]=useState(false);
  const messagesEndRef=useRef(null);
  const recognitionRef=useRef(null);

  const objectionLibrary=data.objectionLibrary||[];
  const cats=["All",...[...new Set(OBJECTION_SCENARIOS.map(s=>s.cat))],"Custom"];
    ...
    try{
      const res=await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({
          ...
          max_tokens:1000,
          system:PRIMERICA_CONTEXT,
          messages:[{role:"user",content:`Start the roleplay. The scenario is: "${scenario.title}". Deliver the opening objection naturally as the prospect would say it in a real conversation. Just say the objection — nothing else.`}]
        })
      });
      const d=await res.json();
      const text=d.content?.[0]?.text||scenario.prospect;
      setMessages([{role:"prospect",text:text.replace("[CONVERSATION_COMPLETE]","").trim()}]);
    }catch(e){
      setMessages([{role:"prospect",text:scenario.prospect}]);
    }
    setIsLoading(false);
  };

  const sendResponse=async()=>{
    if(!userInput.trim()||isLoading||conversationComplete) return;
    const newMessages=[...messages,{role:"rep",text:userInput.trim()}];
    ...
    try{
      ...
      const res=await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({
          model:"claude-sonnet-4-6",
          max_tokens:1000,
          system:PRIMERICA_CONTEXT+`\n\nScenario: ${selectedScenario.title}\nInitial objection: ${selectedScenario.prospect}`,
          messages:apiMessages
        })
      });
      const d=await res.json();
      const text=d.content?.[0]?.text||"I see...";
      const isComplete=text.includes("[CONVERSATION_COMPLETE]");
      const cleanText=text.replace("[CONVERSATION_COMPLETE]","").trim();
      setMessages([...newMessages,{role:"prospect",text:cleanText}]);
      if(isComplete) setConversationComplete(true);
    }catch(e){
      setMessages([...newMessages,{role:"prospect",text:"Let me think about that..."}]);
    }
    setIsLoading(false);
  };
  ...
  const getFeedback=async()=>{
    ...
    try{
      const transcript=messages.map(m=>`${m.role==="rep"?"Rep":"Prospect"}: ${m.text}`).join("\n");
      const res=await fetch("https://api.anthropic.com/v1/messages",{
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({
          model:"claude-sonnet-4-6",
          max_tokens:1000,
          system:`You are a Primerica field trainer giving coaching feedback on an objection handling roleplay session. Be specific, encouraging, and practical. Know that Primerica sells term life insurance only, serves middle-income families, uses the FNA process, and reps are 1099 contractors. Give feedback in this exact JSON format: {"score":8,"whatWorked":["point1","point2"],"toImprove":["point1"],"suggestedResponse":"A better way to say it would be...","keyPhrase":"A memorable one-liner they can use"}`,
          messages:[{role:"user",content:`Scenario: ${selectedScenario.title}\n\nTranscript:\n${transcript}\n\nProvide coaching feedback as JSON only.`}]
        })
      });
      const d=await res.json();
      ...
      setFeedback({score:7,whatWorked:["You engaged with the prospect"],toImprove:["Try to be more specific to their situation"],suggestedResponse:"Great question — let me share what I've seen work for families like yours...",keyPhrase:"Protect what matters most"});
    }
  ...
  const startVoice=()=>{
    if(!("webkitSpeechRecognition" in window||"SpeechRecognition" in window)){
      alert("Voice input isn't supported on this browser. Try Chrome.");
      return;
    }
  ...
  const generateLibraryEntry=async()=>{
    if(!libraryInput.objection) return;
    ...
    try{
      const res=await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({
          model:"claude-sonnet-4-6",
          max_tokens:1000,
          system:`You are a Primerica field training expert. Create a training library entry for handling a specific objection. Return JSON only in this format: {"title":"Short title","category":"Life Insurance|Investments|Recruiting|Other","primaryResponse":"The main response to use","variations":["variation1","variation2"],"keyPhrase":"memorable one-liner","whatNotToSay":"Common mistake to avoid","context":"When this objection typically comes up"}`,
          messages:[{role:"user",content:`Objection: "${libraryInput.objection}"\n${libraryInput.response?`Preferred response from our team: "${libraryInput.response}"`:"Generate the best Primerica-appropriate response."}\n\nCreate a training library entry as JSON only.`}]
        })
      });
      const d=await res.json();
      const text=d.content?.[0]?.text||"{}";
      ...
      const newEntry={...entry,id:Date.now(),addedBy:session.name,addedAt:new Date().toLocaleDateString(),approved:false,objection:libraryInput.objection};
      onUpdate({...data,objectionLibrary:[...objectionLibrary,newEntry]});
      setLibraryInput({objection:"",response:""});
    }catch(e){console.error(e);}
    setGeneratingLibrary(false);
  };

  // HOME screen
  if(mode==="home") return <div style={{padding:dv(16,24),maxWidth:700,margin:"0 auto"}}>
    <div style={{marginBottom:20}}>
      <div style={{fontSize:dv(20,26),fontWeight:800,color:C.text,marginBottom:4}}>🎯 Objection Training</div>
      <div style={{fontSize:13,color:C.textMid}}>Practice handling real objections with AI. Choose a scenario, respond by voice or text, and get coaching at the end.</div>
    </div>
    <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
      {cats.map(c=><button key={c} onClick={()=>setFilterCat(c)} style={{padding:"5px 12px",borderRadius:20,border:`1px solid ${filterCat===c?C.teal:C.border}`,background:filterCat===c?C.teal:"white",color:filterCat===c?"white":C.textMid,fontSize:12,cursor:"pointer",fontWeight:filterCat===c?600:400}}>{c}</button>)}
    </div>
    <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:20}}>
      {OBJECTION_SCENARIOS.filter(s=>filterCat==="All"||s.cat===filterCat).map(s=><button key={s.id} onClick={()=>startScenario(s)} style={{padding:"12px 14px",borderRadius:10,border:`1px solid ${C.border}`,background:"white",cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:12}}>
        <span style={{fontSize:24}}>{s.emoji}</span>
        <div>
          <div style={{fontSize:13,fontWeight:600,color:C.text}}>{s.title}</div>
          <div style={{fontSize:11,color:C.textMid}}>{s.cat}</div>
        </div>
        <svg style={{marginLeft:"auto",flexShrink:0}} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={C.textLight} strokeWidth="2"><polyline points="9 18 15 12 9 6"/></svg>
      </button>)}
    </div>
    <div style={{borderTop:`1px solid ${C.border}`,paddingTop:16}}>
      <div style={{fontSize:14,fontWeight:700,color:C.text,marginBottom:4}}>📚 Objection Library</div>
      <div style={{fontSize:12,color:C.textMid,marginBottom:12}}>Add a real objection you've encountered. AI will build a full training entry your whole team can learn from.</div>
      <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:10}}>
        <textarea placeholder="What was the objection? (e.g. 'My neighbor told me Primerica is a scam')" value={libraryInput.objection} onChange={e=>setLibraryInput({...libraryInput,objection:e.target.value})} rows={2} style={{padding:"8px 10px",borderRadius:8,border:`1px solid ${C.border}`,fontSize:12,color:C.text,resize:"vertical"}}/>
        <textarea placeholder="Optional: What response has worked well for your team? (AI will use this as the foundation)" value={libraryInput.response} onChange={e=>setLibraryInput({...libraryInput,response:e.target.value})} rows={2} style={{padding:"8px 10px",borderRadius:8,border:`1px solid ${C.border}`,fontSize:12,color:C.text,resize:"vertical"}}/>
        <button onClick={generateLibraryEntry} disabled={!libraryInput.objection||generatingLibrary} style={{padding:"9px",borderRadius:8,background:libraryInput.objection&&!generatingLibrary?C.teal:C.textLight,color:"white",border:"none",cursor:libraryInput.objection&&!generatingLibrary?"pointer":"default",fontSize:12,fontWeight:600}}>
          {generatingLibrary?"🤖 AI is building your entry...":"✨ Generate Library Entry"}
        </button>
      </div>
      {objectionLibrary.length>0&&<div>
        <div style={{fontSize:12,fontWeight:700,color:C.textMid,marginBottom:8}}>{objectionLibrary.length} saved {objectionLibrary.length===1?"entry":"entries"}</div>
        {objectionLibrary.slice().reverse().map((entry,i)=><div key={i} style={{borderRadius:8,border:`1px solid ${C.border}`,padding:"10px 12px",marginBottom:8}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
            <div style={{fontSize:12,fontWeight:700,color:C.text}}>{entry.title||entry.objection}</div>
            <Badge color={C.teal} small>{entry.category||"Custom"}</Badge>
          </div>
          <div style={{fontSize:11,color:C.textMid,marginBottom:6,fontStyle:"italic"}}>"{entry.objection}"</div>
          <div style={{fontSize:11,color:C.text,marginBottom:4}}><strong>Primary Response:</strong> {entry.primaryResponse}</div>
          {entry.keyPhrase&&<div style={{fontSize:11,color:C.gold,fontWeight:600}}>💡 Key Phrase: "{entry.keyPhrase}"</div>}
          {entry.whatNotToSay&&<div style={{fontSize:11,color:C.danger,marginTop:4}}>⚠️ Don't say: {entry.whatNotToSay}</div>}
          <div style={{fontSize:10,color:C.textLight,marginTop:6}}>Added by {entry.addedBy} · {entry.addedAt}</div>
        </div>)}
      </div>}
    </div>
  </div>;
  ...
  // PRACTICE screen
  if(mode==="practice") return <div style={{display:"flex",flexDirection:"column",height:"100vh",maxWidth:600,margin:"0 auto"}}>
    <div style={{padding:"12px 16px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:10,background:"white",flexShrink:0}}>
      <button onClick={()=>{setMode("home");setMessages([]);setSelectedScenario(null);}} style={{background:"none",border:"none",cursor:"pointer",color:C.textMid,fontSize:20}}>←</button>
      <div style={{flex:1}}>
        <div style={{fontSize:13,fontWeight:700,color:C.text}}>{selectedScenario?.emoji} {selectedScenario?.title}</div>
        <div style={{fontSize:11,color:C.textMid}}>{selectedScenario?.cat} · Practice Mode</div>
      </div>
      {conversationComplete&&!feedback&&<button onClick={getFeedback} disabled={loadingFeedback} style={{padding:"6px 12px",borderRadius:7,background:C.gold,color:"white",border:"none",cursor:"pointer",fontSize:12,fontWeight:700}}>
        {loadingFeedback?"Analyzing...":"Get Coaching →"}
      </button>}
    </div>
    <div style={{flex:1,overflowY:"auto",padding:"16px",display:"flex",flexDirection:"column",gap:10,background:C.surface}}>
      {messages.map((m,i)=><div key={i} style={{display:"flex",justifyContent:m.role==="rep"?"flex-end":"flex-start"}}>
        <div style={{maxWidth:"80%",padding:"10px 14px",borderRadius:m.role==="rep"?"16px 16px 4px 16px":"16px 16px 16px 4px",background:m.role==="rep"?C.teal:"white",color:m.role==="rep"?"white":C.text,fontSize:13,lineHeight:1.5,boxShadow:"0 1px 4px rgba(0,0,0,0.08)"}}>
          {m.role==="prospect"&&<div style={{fontSize:10,color:C.textLight,marginBottom:3,fontWeight:600}}>PROSPECT</div>}
          {m.text}
        </div>
      </div>)}
      {isLoading&&<div style={{display:"flex",justifyContent:"flex-start"}}>
        <div style={{padding:"10px 14px",borderRadius:"16px 16px 16px 4px",background:"white",fontSize:13,color:C.textLight}}>typing...</div>
      </div>}
      {conversationComplete&&!loadingFeedback&&<div style={{textAlign:"center",padding:"16px",background:C.gold+"11",borderRadius:10,border:`1px solid ${C.gold}33`}}>
        <div style={{fontSize:13,fontWeight:700,color:C.gold,marginBottom:4}}>Conversation Complete!</div>
        <div style={{fontSize:12,color:C.textMid}}>Tap "Get Coaching" above to see your feedback.</div>
      </div>}
      <div ref={messagesEndRef}/>
    </div>
    {!conversationComplete&&<div style={{padding:"12px 16px",borderTop:`1px solid ${C.border}`,background:"white",flexShrink:0}}>
      <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
        <textarea value={userInput} onChange={e=>setUserInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendResponse();}}} placeholder="Type your response or tap the mic to speak..." rows={2} style={{flex:1,padding:"8px 10px",borderRadius:10,border:`1px solid ${C.border}`,fontSize:13,color:C.text,resize:"none"}}/>
        <button onClick={startVoice} style={{width:44,height:44,borderRadius:22,background:isListening?C.danger:C.teal+"22",border:`2px solid ${isListening?C.danger:C.teal}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={isListening?C.danger:C.teal} strokeWidth="2"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2M12 19v4M8 23h8"/></svg>
        </button>
        <button onClick={sendResponse} disabled={!userInput.trim()||isLoading} style={{width:44,height:44,borderRadius:22,background:userInput.trim()&&!isLoading?C.teal:C.textLight,border:"none",cursor:userInput.trim()&&!isLoading?"pointer":"default",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
      ...
      {isListening&&<div style={{fontSize:11,color:C.danger,marginTop:6,textAlign:"center"}}>🔴 Listening... speak now</div>}
    </div>}
  </div>;

  // REVIEW / FEEDBACK screen
  if(mode==="review"&&feedback) return <div style={{padding:dv(16,24),maxWidth:600,margin:"0 auto"}}>
    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}>
      <button onClick={()=>setMode("home")} style={{background:"none",border:"none",cursor:"pointer",color:C.textMid,fontSize:20}}>←</button>
      <div style={{fontSize:dv(18,22),fontWeight:800,color:C.text}}>Coaching Feedback</div>
    </div>
    <Card style={{marginBottom:12,background:`linear-gradient(135deg,${C.navy},#16304f)`,border:"none"}}>
      <div style={{textAlign:"center",padding:"8px 0"}}>
        <div style={{fontSize:48,fontWeight:900,color:C.gold}}>{feedback.score}<span style={{fontSize:20,color:"rgba(255,255,255,0.4)"}}>/10</span></div>
        <div style={{fontSize:13,color:"rgba(255,255,255,0.7)"}}>Overall Score — {selectedScenario?.title}</div>
      </div>
    </Card>
    <Card style={{marginBottom:12,border:`1px solid ${C.success}33`}}>
      <div style={{fontSize:12,fontWeight:700,color:C.success,marginBottom:8}}>✅ What Worked</div>
      {(feedback.whatWorked||[]).map((w,i)=><div key={i} style={{fontSize:12,color:C.text,padding:"4px 0",borderBottom:i<feedback.whatWorked.length-1?`1px solid ${C.border}`:"none"}}>• {w}</div>)}
    </Card>
    {(feedback.toImprove||[]).length>0&&<Card style={{marginBottom:12,border:`1px solid ${C.gold}33`}}>
      <div style={{fontSize:12,fontWeight:700,color:C.gold,marginBottom:8}}>💡 To Sharpen</div>
      {(feedback.toImprove||[]).map((w,i)=><div key={i} style={{fontSize:12,color:C.text,padding:"4px 0",borderBottom:i<feedback.toImprove.length-1?`1px solid ${C.border}`:"none"}}>• {w}</div>)}
    </Card>}
    {feedback.suggestedResponse&&<Card style={{marginBottom:12,border:`1px solid ${C.teal}33`}}>
      <div style={{fontSize:12,fontWeight:700,color:C.teal,marginBottom:8}}>🎯 Suggested Response</div>
      <div style={{fontSize:12,color:C.text,fontStyle:"italic",lineHeight:1.6}}>"{feedback.suggestedResponse}"</div>
    </Card>}
    {feedback.keyPhrase&&<Card style={{marginBottom:16,background:C.gold+"11",border:`1px solid ${C.gold}33`}}>
      <div style={{fontSize:11,fontWeight:700,color:C.gold,marginBottom:4}}>⚡ Key Phrase to Remember</div>
      <div style={{fontSize:14,fontWeight:700,color:C.text}}>"{feedback.keyPhrase}"</div>
    </Card>}
    <div style={{display:"flex",gap:8}}>
      <button onClick={()=>startScenario(selectedScenario)} style={{flex:1,padding:"11px",borderRadius:10,background:C.teal,color:"white",border:"none",cursor:"pointer",fontSize:13,fontWeight:700}}>Try Again</button>
      <button onClick={()=>{setMode("home");setMessages([]);setSelectedScenario(null);setFeedback(null);}} style={{flex:1,padding:"11px",borderRadius:10,background:"white",color:C.text,border:`1px solid ${C.border}`,cursor:"pointer",fontSize:13,fontWeight:600}}>New Scenario</button>
    </div>
  </div>;
...
export default function App() {
  ...
  const renderContent=()=>{
    ...
    if(section==="objectiontraining") return <ObjectionTrainingPage data={data} onUpdate={upd} session={session}/>;    if(section==="quickmsg") return <QuickMessages data={data} onUpdate={upd} userRole={session.role}/>;
